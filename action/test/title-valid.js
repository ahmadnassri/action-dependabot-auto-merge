...
test('title -> parse', async assert => {
  ...
  const command = parse('chore(deps): bump api-problem from 6.1.2 to 6.1.4 in /path', { target: 'patch' })
  ...
  assert.equal(core.info.getCall(3).args[0], 'dependency update target is patch, will auto-merge')
